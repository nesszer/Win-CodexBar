...
mod tests {
    ...
    #[test]
    fn unavailable_status_is_actionable_and_keeps_reset_message() {
        let mut fixture = list_fixture();
        fixture["accounts"][2]["usageStatus"] = json!("unavailable");
        let parsed = parse_account_list(&fixture.to_string()).unwrap();
        let row = parsed.accounts.iter().find(|a| a.number == 3).unwrap();
        assert_eq!(row.usage_status, ClaudeSwapUsageStatus::Unavailable);
