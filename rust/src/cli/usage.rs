...
mod tests {
    ...
    #[test]
    fn claude_swap_payloads_preserve_distinct_same_email_slots() {
        let first = sample_swap_account();
        let mut second = sample_swap_account();
        second.id = "claude-swap:5".to_string();
        second.slot = 5;
        second.label = "Work".to_string();

        let first_payload = claude_swap_json_payload(&first);
        let second_payload = claude_swap_json_payload(&second);

        assert_eq!(
            first_payload["account"]["email"],
            second_payload["account"]["email"]
        );
        assert_eq!(first_payload["account"]["id"], "claude-swap:2");
        assert_eq!(second_payload["account"]["id"], "claude-swap:5");
        assert_ne!(
            first_payload["account"]["id"],
            second_payload["account"]["id"]
        );
    }

    #[test]
    fn claude_swap_unavailable_usage_renders_error_and_empty_windows() {
        let mut account = sample_swap_account();
        account.status = "unavailable".to_string();
        account.error = Some("Polling deferred until a limit resets.".to_string());
        account.five_hour = None;
        account.seven_day = None;
        account.scoped.clear();

        let text = render_claude_swap_text(&account);
        assert!(text.contains("claude-swap"));
        assert!(text.contains("Polling deferred until a limit resets."));
        assert!(!text.contains("Session"));

        let payload = claude_swap_json_payload(&account);
        assert_eq!(payload["account"]["status"], "unavailable");
        assert!(payload["account"]["fiveHour"].is_null());
    }
